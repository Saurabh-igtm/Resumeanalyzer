...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    summary: {
      type: "string",
      description:
        "One-sentence holistic verdict on the resume (max 30 words).",
    },
    strengths: {
      type: "array",
      items: { type: "string" },
      description:
        "Two to four concrete things the candidate did well. Reference specifics.",
    },
    weaknesses: {
      type: "array",
      items: { type: "string" },
      description:
        "Two to four concrete weaknesses with one-line rationale each.",
    },
    rewrites: {
      type: "array",
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (typeof resumeText !== "string") {
    return res
      .status(400)
      .json({ error: "Field `resumeText` (string) is required." });
  }
  ...
  if (trimmed.length > 30_000) {
    return res.status(400).json({
      error:
        "Resume text is too long (max 30,000 characters). Trim and try again.",
    });
  }
    ...
    if (!geminiRes.ok) {
      ...
      if (geminiRes.status === 400 || geminiRes.status === 403) {
        return res.status(geminiRes.status).json({
          error:
            "Gemini rejected the request — likely an invalid API key. Check GEMINI_API_KEY.",
        });
      }
      return res
        .status(502)
        .json({ error: "AI provider failed. Try again in a moment." });
    }
